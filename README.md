...
### 3.5 自适应模式（WARMUP / LAZY / ALERT）

当前版本的自适应模式为自动判模状态机：

- `WARMUP`：严格执行并建立基线参考，验证“高重复度”。
- `LAZY`：复用步骤参考并执行低耗守卫抽检。
- `ALERT`：当守卫异常或到周期复核点时，执行一次严格复核。
- `STRICT_ONLY`：警戒复核失败后降级。

关键说明：

- 勾选“启用自适应模式”后，模式自动切换，不依赖手工 strict/lazy。
- 参考块以“每个步骤首次严格成功”作为固定基线，不会每轮覆盖。
- 守卫每轮随机挑选 1 个步骤，再随机抽样多个块投票判定。
- 所有动作步骤都纳入守卫与参考（`ClickText` / `ClickImage` / `ClickPosition` / `TypeText` / `KeyPress` / `Wait`）。

### 3.6 守卫落盘审计（`save/guard_debug/`）

为了可追溯调试，运行时会把守卫数据落盘到会话目录：
...
- `reference/`：步骤基线参考块（首次严格成功生成）。
- `checks/`：每次守卫抽检的实时块与报告。
- 每次抽检会生成 `report.json` 与 `report.txt`，包含：
  - 抽检序号
  - 抽样块索引
  - 每块前后相似度
  - 投票结果与总判定
  - 对应参考目录路径（`reference_path`）
